...
if (isset($_POST["submit"])) {
    ...
    if ($_FILES['p_photos']['error'][0] === UPLOAD_ERR_NO_FILE) {
        ...
        $stmtToGetOrgPhotos->bindValue(":id", $sent_id);
        ...
        $img_upload_path = '../../../Storage/house/' . $sent_id;
    ...
    // p code related
    $p_code = 'G' . ($p_offer === '0' ? 'R' : 'S') . 'A' . $sent_id;
    // facility
    ...
    SET 
        ...
        p_after = :p_after,
        go_id = :go_id
    WHERE p_code = :p_code AND uploader_id = :uploader_id;
    ";
    ...
    $propertyUpdateStmt->bindValue("id", $sent_id);
    $propertyUpdateStmt->bindValue("uploader_id", $current_collaborator);
    $propertyUpdateStmt->execute();

    // header("Location: PStatusUpgrade.php?id=$propertyID&p_status=2");
    header("Location: ../../View/Property/detail.php?id=$sent_id");
}
